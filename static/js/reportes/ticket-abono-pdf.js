function verTicketAbono(abono_id){
 ...
 /*  doc.setFont("helvetica", "bold"); // set font
  ...
  penalizacion_monto = data.datos.datos_abono.penalizacion_monto
  no_abono = data.datos.datos_abono.no_abono

  DatosGenerales = [{"manzana": manzana,"lote": lote, "tipo": tipo,  "no_abono": no_abono, "mes": mes, "total": total, "year": year, "penalizacion_monto": penalizacion_monto}]
  ...
  doc.autoTable(({
    ...
    columns: [
      ...
      { header: 'Penalización', dataKey: 'penalizacion_monto' },
    ],
  }))
 ...
 /*  doc.text("Pagado $" +  data.datos.pagado,129, 76);
  ...
  const formateado_total_neto = total.toLocaleString("en");
    ...
    const for_penalizacion = !penalizacion ? 0 : penalizacion.toLocaleString("en");
